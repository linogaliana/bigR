...
* **le problème est adapté à la parallélisation**. La parallélisation ne prend sens que dans un cadre où on peut restructurer les données en blocs indépendants. Une somme cumulative sur un vecteur ne sera pas à une situation où la parallélisation apportera un gain (elle peut même ralentir le système). 
...
La comparaison de vitesse pourrait être optionnelle et figurer pour info dans une note déroulable (facile à faire en Markdown, j'ai le code de Martin Chevalier qui fait ça quelque part).
...
CommOM: j'ai créé un paragraphe introductif pour guider le lecteur. Je propose un premier jet à partir de tes éléments (je suis peut-être à côté de la plaque). Attention à bien définir le terme de cluster, et son rapport avec un processus esclave (c'est la même chose ou pas?).
...
* **Le découpage des données à traiter**: On découpe les données en blocs indépendants en fonction d'une variable de groupe;
* **L'initialisation de la parallélisation**: on initialise les processus esclaves et crée une barre de progrès, notamment le choix du nombre de clusters; sans initialisation des *clusters*, un code prévu pour être exécuté en parallèle ne produira pas la parallélisation attendue mais exécutera les tâches séquentiellement;
* **L'exécution parallèle d'une série d'instructions**: on traite chaque bloc de données séparément;
* **La finalisation du traitement parallèle**: un traitement en parallèle se termine par la fermeture des *clusters*.
...
CommOM: je pense que les lecteurs ne pourront pas faire l'exercice sans un exemple complètement résolu (du moins, moi je n'y arriverais pas). Je te propose de résoudre intégralement le cas de la moyenne par groupe, et de faire un exemple pas trop différent (min et max par groupe?).
CommOM: est-ce vraiment utile de parler des options `.multicombine` et `.maxcombine`? Cela risque d'embrouiller le lecteur. Tu pourrais faire une section plus bas "pour aller plus loin dans la parallélisation", en expliquant qu'on peut avoir des outputs plus compliqués, et présenter alors ces options (et présenter aussi les autres `combine` possibles).
...
* `.multicombine = TRUE` et `.maxcombine = TRUE`: permet de sortir une liste, éventuellement à plusieurs niveaux, dont le nombre d'éléments de premier niveau est égal au nombre de groupes;
...
CommOM: je ne modifie pas l'exercice car ça dépend vraiment de ce que tu veux expliquer ici.

**Exercice**: En reprenant la structure proposée précédemment:
...
Cette fois, la solution parallélisée est légèrement plus rapide et demande surtout 2000 fois moins de mémoire. Cela illustre bien la manière dont l'approche *divide and conquer* peut aider en cas de traitements gourmands à limiter l'inflation des besoins en RAM.
...
CommOM: je sépare solutions des problèmes et évaluation de la performance.
...
Voici la solution séquentielle:
<div class="fold s">
```{r, eval = FALSE}
...
Voici l'évaluation de la performance de cette solution avec `profvis`:
...
Voici la solution parallélisée:
