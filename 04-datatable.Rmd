...
`R` est réputé être un langage lent et de nombreux exemples sur internet proposent des comparatifs de vitesse qui suggèrent que `python` est plus rapide (nous avons déjà proposé un petit comparatif dans l'introduction de ce cours). Il est vrai que le langage de base `R` est lent mais le développement de packages tels que `data.table` et `tidyverse` ont pour conséquence que la grammaire de base est de moins en moins utilisée de manière exclusive. Ainsi, pour rendre réellement compte des performances du langage `R`, il convient de mesurer les vitesses d'exécution de ces deux approches. A ce jeu, le package `data.table` est très performant:
...
Autrement dit, **traiter efficacement de grosses bases de données en `R` ne correspond pas aux cas d'usage pour lesquels le `tidyverse` a été développé**. Le package `data.table` est plus rapide mais est aussi moins gourmand en `RAM`. Il s'agit donc d'une solution intéressante pour des bases de données de grosses tailles (comme les données administratives) mais dont le volume reste inférieur à la mémoire disponible. Nous verrons dans le dernier chapitre des solutions *out-of-memory* pour les bases qui ne peuvent être lues en `R` parce que trop volumineuses (ou pour lesquelles des traitements statistiques, par exemple une régression linéaire, amèneraient à saturer la RAM).

CommOM: Je pense que cette partie sur data.table en production est très intéressante, et hyper utile pour les utilisateurs. Tu pourrais peut-être la développer un peu. Un point à reformuler peut-être: veux-tu dire que data.table est extrêmement robuste en général, ou plutôt que l'absence de dépendances fait qu'il est peu probable qu'un code data.table casse suite à une mise à jour, contrairement à l'écosystème tidyverse qui a beaucoup de dépendances?

### Avantages de `data.table` en production par rapport au `tidyverse`**

En premier lieu, `data.table` ne dépend que de `R` base et n'importe que quelques fonctions du package `methods` (présent dans la distribution de base de `R`). Cela signifie que ce package est extrêmement robuste puisque la distribution `R` base est très stable. Le `tidyverse` étant un écosystème très récent avec de nombreuses dépendances, il est beaucoup moins stable que `data.table`. Pour des projets en production, on peut donc se fier à `data.table` ; à condition de faire attention aux effets induits du comportement de passage par référence sur lequel on reviendra. `dplyr` étant un package jeune, il n'est pas aussi stable que `data.table`: certaines fonctionnalités peuvent rapidement ne plus être maintenues au profit d'autres. Un code `dplyr` peut ainsi rapidement être obsolète ou ne plus être fonctionnel car une dépendance ou fonction sous-jacente à évolué.

### Aparté: évaluation standard et évaluation non-standard en `R`

Une deuxième raison incite à privilégier `data.table` plutôt que `dplyr` dans un processus de production. `dplyr` n'est pas d'un langage de programmation fonctionnelle mais une syntaxe visant à faciliter le maniement de données. 

Il existe deux manières de faire référence à un objet en `R`:

* L'évaluation standard: on entre un nom, et R va aller chercher sa valeur en partant de `.GlobalEnv` pour descendre dans la hiérarchie du `searchpaths()`, jusqu’à trouver une valeur associée à votre symbole. S’il n’en trouve pas, il renvoie une erreur.
* L'évaluation non-standard: `R` contourne la règle de la première recherche dans l'environnement global pour évaluer dans un contexte implicite la valeur derrière le nom.

En évaluation non-standard, l'objet `x` est compris comme faisant partie de l'environnement en question (par exemple le dataframe `df` ou le data.table `dt`). Ce n'est que s'il n'existe pas dans cet environnement que `R` cherchera dans l'environnement global. `data.table` et `dplyr` reposent tous deux sur l'intégration standard (on cite les noms de colonnes sous la forme `dt[,x]` ou `df %>% mutate(x)` ce qui relève de l'intégration standard). 

Un exemple ([emprunté à ThinkR](https://thinkr.fr/tidyeval/)) est sans doute plus explicite. 

```{r, eval = FALSE}
iris %>% dplyr::filter(Species == "setosa")
```

Le symbole `Species` ne réfère à rien si on l’envisage via l’évaluation standard : il n’existe pas de symbole `Species` dans l’environnement global pointant vers une valeur. Ici, le symbole `Species`  doit être évalué dans le contexte du data.frame iris. En base, et donc en évaluation standard, il faudra écrire `iris$Species`:

```{r, eval = FALSE}
iris[iris$Species == "setosa", ]
```

Comme `dplyr`, `data.table` fonctionne bien avec l'évaluation non standard. L'équivalent `data.table` de la ligne précédente peut s'écrire:

```{r, eval = FALSE}
data.table::as.data.table(iris)[Species == "setosa"]
```

Cette évaluation non standard est très pratique. Mais, dans une optique de programmation fonctionnelle, ou même lors d'un partage de code, elle peut devenir problématique car les objets dans l'environnement global d'un utilisateur sont inconnus. 

Par exemple, les deux commandes ci-dessous ne vont pas produire le même résultat car il y a ambiguïté sur `Species`: parle-t-on de la variable ou de l'objet ?
...
L'intégration de l'évaluation standard, qui est à la fois pratique dans une approche de programmation fonctionnelle (on peut passer des noms de variable en argument d'une fonction) et désirable pour la reproductibilité des programmes est ardue à mettre en oeuvre dans `dplyr` alors qu'elle est très simple en `data.table`^[Si vous ne me croyez pas, essayer de comprendre du premier coup la [vignette dplyr](https://dplyr.tidyverse.org/articles/programming.html) sur le sujet]. 

Il est possible d'appliquer de l'évaluation standard de trois manières en `data.table`. La première, probablement la plus pratique, est basée sur la fonction de base `get`. Il s'agit de mettre le nom de la variable en caractère et l'appeler dans les crochets avec `get`:
...
L'approche équivalente de `dplyr`, fondée sur `rlang` est beaucoup moins pratique.

Les deux autres manières sont présentées ici mais ne seront pas approfondies^[Les personnes familières avec la syntaxe `unix` retrouveront une similarité entre les `..` en `data.table` et sur le terminal. C'est le même principe qui s'applique: `..` signifie "un niveau au-dessus": `..` signale à `data.table` de rechercher les variables `select_cols` un niveau au-dessus, i.e. dans l'environnement global dans ce cas.]:
...
### Le `data.table`, un `data.frame` amélioré

Le package `data.table` propose une version améliorée du `data.frame` de base: le `data.table` (surprenant, non?). On peut **convertir** un objet en data.table en utilisant `data.table::setDT()` ou `data.table::as.data.table()`. La première méthode est préférable, nous expliquerons par la suite pourquoi. Pour **créer** un data.table, on utilisera la fonction `data.table::data.table()` comme on utiliserait la fonction `data.frame()` pour créer un dataframe. Comme avec un `tibble`, la visualisation est améliorée par rapport à un `data.frame` standard:
...
### La syntaxe de `data.table`: l'opérateur `[...]`
...
Par exemple, pour effectuer une filtre et une moyenne par groupe sur les données d'exemple ci-dessous,
...
Cette syntaxe est beaucoup moins bavarde que celle du `tidyverse` et est donc déroutante pour les débutants. Pourtant, elle n'est pas forcément moins claire. Notez qu'on a appliqué des fonctions `dplyr` sur un `data.table` mais ce n'est pas une pratique recommandable: les 5 verbes du `tidyverse` ont une traduction correspondante en `data.table`, plus adaptée pour ces objets. 
...
if (!file.exists("~/data/sirus_abstract.csv")) {
  df <- data.table::fread('https://github.com/SSP-Lab/Hackathon-2018')
  data.table::fwrite(
...
1.  Importer les données avec chacune des méthodes suivantes (n'hésitez pas à consulter l'aide de chaque fonction pour obtenir le nom des arguments):
a.  `read.csv` en faisant attention à ne pas oublier l'option `stringsAsFactors = FALSE` et l'argument `sep`;
b.  `readr::read_csv` en désactivant la barre de progrès;
c.  `data.table::fread` en désactivant également la barre de progrès;
2.  Faire un profiling avec la fonction `profvis::profvis` de chacune des méthodes.
...
Voici la solution pour importer les données:
<div class="fold s">
```{r, eval = FALSE}
...
Voici la comparaison des besoins en mémoire:
...
La fonction `data.table::fread()` est à la fois:
...
La fonction de base `read.csv` est très lente. Pourtant, elle est codée en `C` comme la fonction de `data.table`. Le principal problème de la fonction de base (que les fonctions `readr` et `fread` évitent) est qu'elle importe l'ensemble du fichier sous forme `character` et convertit *a posteriori* les colonnes pouvant l'être sous des formats numériques. Les fonctions `read_csv` et `fread` inspectent le fichier avant de l'importer (les premières lignes pour `read_csv` ; les premières et dernières lignes pour `fread`).
...
2. Utiliser l'option `drop` et le vector `cols` pour éliminer les colonnes qui commencent par `adr_` ou `eff_`. Vous pouvez utiliser, pour filtrer `cols`: une expression régulière insérée dans `grepl` ou la fonction de base `startsWith`. Si vous êtes à l'aise avec le package `stringr` ou `stringi` vous pouvez les utiliser.
...
* Formats `sas`: pas de miracle, il faut utiliser la fonction `haven::read_sas` et convertir en `data.table` avec `data.table::setDT` function. `haven` intègre depuis peu des paramètres qui peuvent être précieux pour réduire les besoins mémoires et CPU lors de l'import: `col_select` et `n_max`.


### Comparaison des écritures

Partons des dataframe suivants:
...
La fonction pour écrire des données en fichier plat est `fwrite` (*fast write*), alternative à `readr::write_csv`.

**Exercice**

1. Comparer les temps d'exécution sur 20 essais avec `microbenchmark`
...
2. Regarder les besoins mémoire avec `profvis`
...
La fonction `tracemem` peut être utilisé pour suivre l'évolution des emplacements mémoire des objets `R`
...
On va partir des données `sirus` complètes. 
...
Les requêtes sur la dimension `i` peuvent servir à créer de nouvelles colonnes. Dans ce cas, on peut penser la dimension `i` comme un `update` en SQL ou un `case when`. Dans le tidyverse, la manière adaptée d'effectuer un remplacement conditionnel est d'utiliser `dplyr::case_when`. C'est possible en `data.table` mais il y a mieux grâce au remplacement conditionnel.

Par exemple, pour créer une variable `iledefrance` qui vaut `TRUE` pour un département appartenant à la région, avec `data.table` on fera de la manière suivante:
...
**Exercice**
1. Mesurer les besoins mémoire et le temps d'exécution grâce à `profvis`
2. Faire la même chose pour le code équivalent avec `dplyr::case_when`
3. Conclure
...
## Appariemment: la syntaxe *merge*



## Indexation: accélérer les opérations par groupe, les filtres et les appariemments

On part d'un `data.frame` de taille conséquente en prenant `sirus` dans son ensemble.

```{r}
df <- data.table::fread("~/data/sirus_abstract.csv",
                        showProgress = FALSE)
```

**NB: revoir avec la différence entre index et key en data.table**

Les dataframes de base ont un attribut qui est le `rowname` et permet de manière unique d'identifier des lignes. Cette fonctionalité n'est pas proposée par `data.table` (ni par `dplyr`) car il existe un principe beaucoup plus puissant: les index^[Dans ce cours, nous n'évoquerons que les index primaires. Il y a en fait, comme en SQL, la possibilité d'avoir des index secondaires avec un `data.table`. La différence est qu'il ne s'agit pas de réordonner les données dans des espaces contigües comme avec des index mais plutôt de stocker quelque part un vecteur d'ordre. L'avantage est qu'on peut, dès lors, utiliser plusieurs index secondaires alors qu'on ne peut avoir qu'un index primaire]. Un index en `data.table`, contrairement à un rowname de dataframe:

* Un index peut être construit à partir de plusieurs colonnes, dont le type peut différer
* L'unicité de l'indice n'est pas nécessaire. Ce sera pratique lors des opérations `by`. Les observations dont la clé est identique apparaîtront dans des observations consécutives. 

Fixer une clé a deux conséquences:
* Reordonne (ordre croissant) en mémoire le `data.table` en fonction des colonnes passées par référence. Cela permettra d'accélérer grandement les opérations sur des lignes
* ajoute un attribut `sorted` au `data.table`

Comme la table est modifiée par référence, l'action de fixer une clé est très efficient en mémoire. Elle permet de gros gains de temps lors des opérations sur les lignes mais a un coût fixe qui est le temps d'initialisation de l'index lors de l'appel de la commande. 

Comme le mentionne l'aide de `setkey` (`?setkey`), il y a trois raisons pour lesquelles il est utile d'utiliser des indices:

* On peut utiliser un algorithme de recherche binaire (*binary search*) plutôt qu'un algorithme de parcours de vecteur (*vector scan*) pour les recherches sur les lignes et les joins. L'algorithme de recherche binaire est beaucoup plus efficace que l'algorithme de parcours de vecteur^[La recherche binaire consiste à comparer la valeur recherchée au milieu du vecteur de recherche (ordonné). Dans le cas où il n'y a pas égalité, la moitié à laquelle l'élément ne peut appartenir est éliminée et on prend à nouveau l'élément au milieu et ainsi de suite jusqu'à trouver (ou non) l'élément recherché. Dans le pire des cas, la complexité de l'algorithme est `O(log(n))` ce qui en fait un bon algorithme de recherche pour des vecteurs de taille conséquente. L'opération d'ordonnancement du vecteur de recherche est de complexité `O(n)` ], en particulier pour des données où la dimension $n$ est conséquente.
* Les opérations de groupe sont accélérées parce que les groupes sont déjà présents dans des espaces de RAM contigüs
* On peut faire des requêtes plus courtes pour faire du subsetting sur des dimensions. Par exemple `DT["id",]` trouve, par recherche binaire, le groupe `id` dans la première colonne qui sert de clé à `DT`


### Créer un index ou une clé

On dispose des fonctions suivantes:

* `setkey` ou `setkeyv` permettent de définir une clé. Les deux fonctions différent dans le format accepté pour le nom de colonnes 
* `setindex` ou `setindexv` permettent de définir un indice. Les deux fonctions différent dans le format accepté pour le nom de colonnes

Les fonctions terminant par `*v` s'appuient sur l'évaluation standard, i.e. des noms de colonnes sous la forme `c("col1","col2")` alors que les fonctions ne terminant pas par `v` reposent sur le principe de l'évaluation non standard, donc des noms de colonnes sous la forme  `c(col1,col2)`

Par exemple, si on désire fixer les colonnes `region` et `apet`, on utilisera le code suivant:

```{r}
df_key <- data.table::copy(df)

# METHODE 1: SE
data.table::setkeyv(df_key, cols = c("region", "apet"))
data.table::key(df_key)
data.table::haskey(df_key)

# METHODE 2: NSE
data.table::setkey(df_key, region, apet)
data.table::key(df_key)
data.table::haskey(df_key)

# NB: POUR SUPPRIMER UNE CLE SANS LA REMPLACER
data.table::setkeyv(df_key, cols = NULL)
data.table::key(df_key)
data.table::haskey(df_key)

# ON SUPPRIME LE DATA.TABLE TEMPORAIRE
rm(df_key)
```

Et de même pour les index avec les fonctions appropriées. Comme toutes les fonctions de la famille `set*`, on n'a pas besoin de faire d'assignation: la modification a lieu par référence. 



### Accélérer le subsetting sur les lignes

Le fait d'avoir ordonné le `data.table` (explicitement avec les clés ou implicitement avec les index) permettra d'accélérer grandement les opérations de filtre grâce à la recherche binaire.

** Exercice**

1. Repartir de `df` et vérifiez qu'il n'a pas de clé ou d'index. S'il en a, utiliser le code précédent pour la retirer. Créez plusieurs copies de `df`: `df_key`, `df_index`. Créez aussi `df_tbl` avec la fonction `dplyr::as_tibble`
2. Fixer les colonnes `region` et `apet` comme clé pour `df_key` et comme indice pour `df_index`
3. Faire un microbenchmark différent des deux propositions ci-dessous sur la solution data.table non indexé, indexé, avec clé et sur un tibble:

* Garder uniquement les observations où `region %in% c(75,77,78,91,92,93,94,95)`
* Garder uniquement les observations telles que `region %in% c(75,77,78,91,92,93,94,95)` et `apet == "0111Z"`  

```{r}
# Question 1
if (data.table::haskey(df)) data.table::setkey(df, NULL)
df_key <- data.table::copy(df)
df_index <- data.table::copy(df)
df_tbl <- dplyr::as_tibble(df)


# Question 2
data.table::setkeyv(df_key, c("region","apet"))
data.table::setindexv(df_index, c("region","apet"))

import::from("magrittr","%>%")

# Question 3
m1 <- microbenchmark::microbenchmark(
  df[region %in% c(75,77,78,91,92,93,94,95)],
  df_key[region %in% c(75,77,78,91,92,93,94,95)],
  df_index[region %in% c(75,77,78,91,92,93,94,95)],
  df_tbl %>% dplyr::filter(region %in% c(75,77,78,91,92,93,94,95)),
  times = 20L
)
m1
ggplot2::autoplot(m1)
...
m2 <- microbenchmark::microbenchmark(
  df[region %in% c(75,77,78,91,92,93,94,95) & apet == "0111Z"],
  df_key[region %in% c(75,77,78,91,92,93,94,95) & apet == "0111Z"],
  df_index[region %in% c(75,77,78,91,92,93,94,95) & apet == "0111Z"],
  df_tbl %>% dplyr::filter(region %in% c(75,77,78,91,92,93,94,95) & apet == "0111Z"),
  times = 20L
)
m2
ggplot2::autoplot(m2)
...
Conclusion: indexation rend beaucoup plus rapide `data.table`. D'autant plus précieux qu'on a plus de colonne (ce qu'on a testé) ou qu'on a plus de lignes ($O(log(n))$ vs $O(n)$).

La [vignette](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-secondary-indices-and-auto-indexing.html) sur le sujet des indices offre plus de détails sur les possibilités de simplifier les syntaxes de subsetting lorsqu'on a des indices. Le lecteur intéressé pourra s'y référer. 
...
Faire des opérations sur un `data.table` dont les clés ont été fixées est beaucoup moins gourmand en mémoire. 


### Agrégations efficaces

Les opérartions de groupement sont facilitées par l'indexation. C'est particulièrement utile pour des données multiniveaux

**Exercice**

1. Reprendre les 4 bases précédentes `df`, `df_key`, `df_index` et `df_tbl`
2. Faire un microbenchmark (20 itérations)

* Du nombre d'observations par groupe (region et apet)
* Du nombre d'observations par groupe (region et apet), de la somme de la variable `eff_etp_uniteLegale` et de la moyenne de la variable `eff_3112_uniteLegale`

```{r}
m1 <- microbenchmark::microbenchmark(
  df[,.N, by = c("region", "apet")],
  df_key[,.N, by = c("region", "apet")],
  df_index[,.N, by = c("region", "apet")],
  df_tbl %>% dplyr::group_by(region, apet) %>%
...
m2 <- microbenchmark::microbenchmark(
  df[,.(.N,
        sum(eff_etp_uniteLegale, na.rm=TRUE),
        mean(eff_3112_uniteLegale, na.rm = TRUE)),
     by = c("region", "apet")],
  df_key[,.(.N,
            sum(eff_etp_uniteLegale, na.rm=TRUE),
            mean(eff_3112_uniteLegale, na.rm = TRUE)),
...
### Jointures de données efficaces
