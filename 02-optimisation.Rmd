...
CommOM: je pense que cette section pourrait être mis en dernier, ce serait peut-être plus cohérent: 1/ base de la mémoire; 2/ il y a deux comportements de copie donc 3/ les besoins en mémoire d'opérations similaires sont très différents selon le mode de copie donc 4/ la consommation de RAM est difficile à suivre, d'où A/ Le risque de saturation de la RAM, B/ l'utilisation de `htop` et `pryr`, C/ l'importance de faire des `gc()`.
...
CommOM: il me semble utile de commenter dans le texte le code suivant, pour que les lecteurs comprennent bien de quoi il est question. Personnellement, je ne comprends pas tout. Je comprends que la consommation mémoire est difficile à suivre en regardant simplement le code.
