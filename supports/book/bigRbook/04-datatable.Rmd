...
```{r}
# METHODE 1: SE
data.table::setkeyv(df, cols = c("region", "apet"))
data.table::key(df)
data.table::haskey(df)

# METHODE 2: NSE
data.table::setkey(df, region, apet)
data.table::key(df)
data.table::haskey(df)

# NB: POUR SUPPRIMER UNE CLE SANS LA REMPLACER
data.table::setkeyv(df, cols = NULL)
data.table::key(df)
data.table::haskey(df)

```

Et de même pour les index avec les fonctions appropriées. 
