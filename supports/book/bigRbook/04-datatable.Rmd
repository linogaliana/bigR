...
CommOM: j'ai l'impression que tu fais un comparatif un peu long entre `data.table` et `tidyverse`, ça vaut le coup d'en faire une partie.
...
**Avantages de `data.table` en production par rapport au `tidyverse`**: `data.table` ne dépend que de `R` base et n'importe que quelques fonctions du package `methods` (présent dans la distribution de base de `R`). Cela signifie que ce package est extrêmement robuste puisque la distribution `R` base est très stable. Le `tidyverse` étant un écosystème très récent avec de nombreuses dépendances, il est beaucoup moins stable que `data.table`. Pour des projets en production, on peut donc se fier à `data.table` ; à condition de faire attention aux effets induits du comportement de passage par référence sur lequel on reviendra.
...
Le package `data.table` propose une version améliorée du `data.frame` de base: le `data.table` (surprenant, non?). On peut convertir un objet en data.table en utilisant `data.table::setDT()` ou `data.table::as.data.table()`. Pour créer un data.table, on utilisera la fonction `data.table::data.table()` comme on utiliserait la fonction `data.frame()` pour créer un dataframe. Comme avec un `tibble`, la visualisation est améliorée par rapport à un `data.frame` standard:
...
CommOM: je mettrais bien ici un exemple très simple sur un tout petit dataframe, avec l'équivalent `dplyr`. Un truc du genre: dt[x > 3, sum(y), by = z] et df %>% filter(x > 3) %>% group_by(z) %>% summarise(sum(y)).

Cette syntaxe est beaucoup moins bavarde que celle du `tidyverse` et est donc déroutante pour les débutants.
