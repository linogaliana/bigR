...
A cause du comportement de copie, il faut faire attention à l'allocation de mémoire lorsqu'on désire ajouter des éléments à un objet. Comme nous av

allons le voir, il est préférable de pré-allouer la taille d'un objet plutôt que de le faire croître ou alors utiliser une liste. 
...
* Comparer la performance des trois méthodes avec `microbenchmark::microbenchmark` avec $n=1000$
...
La méthode optimale est bien-sûr celle utilisant la fonction adaptée. La pré-allocation a permis de gagner pas mal de temps. 

* Effectuer un *profiling* avec $n=1e5$
...
La méthode 2, loin d'être efficiente, est 400 fois plus rapide que la méthode 1. @gillespie2016efficient propose plus de résultats en faisant varier la taille du vecteur. Quand $n=10^7$, la méthode 1 prend une heure quand la méthode 2 fonctionne en 2 secondes et la méthode 3 est instantanée. 
