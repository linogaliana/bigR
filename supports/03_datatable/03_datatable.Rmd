...
**Pourquoi faire du `data.table` quand on sait faire du `tidyverse`?**  La citation ci-dessous, issue du [post](https://stackoverflow.com/questions/21435339/data-table-vs-dplyr-can-one-do-something-well-the-other-cant-or-does-poorly) déjà évoqué, souligne bien que les limites du `dplyr` compte tenu de l'objectif de ce cours, qui est de traiter efficacement de grosses bases de données en `R` correspond à un cas d'usage qui ne correspond pas à la construction du `tidyverse`: 
